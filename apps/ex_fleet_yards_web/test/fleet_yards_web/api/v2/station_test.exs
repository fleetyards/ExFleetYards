defmodule ExFleetYardsWeb.Api.V2.StationTest do
  use ExFleetYardsWeb.ConnCase
  import OpenApiSpex.TestAssertions

  describe "Api V2 Game Station" do
    test "show olisar", %{conn: conn} do
      json =
        conn
        |> get_api(ApiRoutes.station_path(conn, :show, "port-olisar"))
        |> json_response(200)
    ...
    test "spec compliance (:index)", %{conn: conn, api_spec: spec} do
      json =
        conn
        |> get_api(ApiRoutes.station_path(conn, :index))
        |> json_response(200)
